...
class BacktestEngine:
    ...
    def __init__(self, initial_equity: float = 10000.0):
        ...
        self._running_equity = initial_equity   # ← initialize here
    ...
    async def run(
        ...
        if open_positions:
            ...
            self.logger.info(f"Closing {len(open_positions)} open position(s) at last price {last_candle.close}")
            ...
            # Update equity after forced closes
    ...
    async def _update_account(self, candle: Candle):
        ...
        total_unrealized = sum(pos.unrealized_pnl(candle.close) for pos in self.position_mgr.get_open_positions())
    ...
    async def _process_signal(self, signal: Signal, candle: Candle, window: List[Candle]):
        ...
        if signal.symbol in self.position_mgr.positions:
            self.logger.debug(f"Already have open position for {signal.symbol}, ignoring new signal")
            return

        side = OrderSide.LONG if signal.signal_type == SignalType.BUY else OrderSide.SHORT
        equity = self._running_equity

        # Use Kelly to compute position size (fallback to 2% risk if no history)
        kelly_fraction = self.kelly.compute_kelly_fraction()
        # Use a default stop distance of 2% of price for sizing
        sl_distance = candle.close * 0.02
        quantity = self.kelly.calculate_position_size(
            equity, candle.close, candle.close - sl_distance
        )
        if quantity < 0.1:  # minimum trade size for SOL
            return

        # Determine stop loss and take profit levels
        mr_confidence = 0.6  # placeholder; could be extracted from hybrid signal
        sl, tp = self.sl_calc.compute_hybrid_levels(window, side, candle.close, mr_confidence)

        # Simulate entry fill with slippage
        fill_price = self.fill_sim.simulate_entry(candle, side, signal.entry_price or candle.close)
    ...
    def _compute_metrics(self) -> BacktestMetrics:
        ...
        avg_loss = self.sum_losses / (self.total_trades - self.winning_trades) if (self.total_trades - self.winning_trades) > 0 else 0
